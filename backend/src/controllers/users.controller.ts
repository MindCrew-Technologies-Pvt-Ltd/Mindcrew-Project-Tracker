...
export const getUsers: RequestHandler = async (req, res, next) => {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.user.findMany({ where, skip, take, orderBy: { createdAt: 'desc' }, select: { id: true, name: true, email: true, phone: true, department: true, designation: true, jobRoles: true, role: true, isActive: true, createdAt: true, updatedAt: true, _count: { select: { ownedProjects: true } } } }),
...
export const getUser: RequestHandler = async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id }, select: { id: true, name: true, email: true, phone: true, department: true, designation: true, jobRoles: true, role: true, isActive: true, createdAt: true, updatedAt: true } });
...
export const updateUser: RequestHandler = async (req, res, next) => {
  try {
    ...
    const { name, phone, department, designation, jobRoles, role, isActive } = req.body;
