...
export const getUser: RequestHandler = async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id }, select: { id: true, name: true, email: true, phone: true, department: true, designation: true, employeeId: true, jobRoles: true, pendingJobRoles: true, managerEmployeeIds: true, role: true, isActive: true, createdAt: true, updatedAt: true } });
...
export const updateUser: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (jobRoles !== undefined) {
      ...
      data.pendingJobRoles = []; // clear pending roles when admin updates roles
    }
