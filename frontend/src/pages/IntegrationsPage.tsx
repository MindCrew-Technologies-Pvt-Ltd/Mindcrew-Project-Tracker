...
const FILL_RULE = `When I say "fill my timesheet": review everything I worked on TODAY across all projects and sessions, match each piece of work to a tracker project via the project-tracker MCP tool \`list_projects\`, and call \`log_work\` once per project. NEVER ask me how many hours to log — estimate honestly from the actual work you observed, even if I suggest a number. Write a DETAILED bullet summary of concrete accomplishments (files, features, fixes) with an approximate duration per bullet, e.g. "- Rebuilt dashboard filters (~1h 30m)" — bullets should roughly add up to the logged time. Mark work you were told about but did not observe, e.g. "(1h — as reported by me)". Call \`get_my_week\` first to avoid double-logging, and if other AI sessions ran in parallel today, split the overlapping clock time between projects — the same hour must never be counted twice. End the summary with a "Time basis:" line showing the arithmetic (active blocks, exclusions) behind the total. Time must be logged the same day — days lock at 11:59 PM IST.`;
