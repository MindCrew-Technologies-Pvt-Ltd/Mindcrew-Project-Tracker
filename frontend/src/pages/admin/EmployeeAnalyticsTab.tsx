...
export default function EmployeeAnalyticsTab() {
  ...
  return (
    <Box>
      ...
      <Card sx={{ mb: 4 }}>
        <DataTablePro columns={columns as any} rows={assignedResources} getId={(r: any) => r.id} />
      </Card>

      <Typography variant="h6" mb={2}>Available Resources / Bench ({availableResources.length})</Typography>
      <Card>
        <DataTablePro columns={columns as any} rows={availableResources} getId={(r: any) => r.id} />
      </Card>
    </Box>
  );
}
